...
%Thais está exatamente à direita da mulher de 25 anos.
               exat_a_direita(A,B,S),
               nome(A,thais),
               idade(B,vinte_e_cinco),

%Paula está na primeira posição.
               primeira_da_fila(C,S),
               nome(C,paula),

%A de Branco está ao lado da mulher que tem Franja.
               ao_lado(D,E,S),
               camiseta(D,branca),
               cabelo(E,franja),
...
%A garota de Rabo de Cavalo está exatamente à direita da que usa Franja.
               cabelo(G,rabo_de_cavalo),
               cabelo(H,franja),
               exat_a_direita(G,H,S),

%A aquariana está exatamente à esquerda da mulher de 26 anos.
               signo(I,aquariana),
               idade(J,vinte_e_seis),
               exat_a_esquerda(I,J,S),

%A mais nova está em algum lugar entre a de 27 anos e a de 25 anos, nessa ordem.
               entre(K,L,M,S),
               idade(K,vinte_e_quatro),
               idade(L,vinte_e_sete),
               idade(M,vinte_e_cinco),

%A mulher de 25 anos foi à farmácia para comprar Desodorante.
               pessoa(N,S),
               idade(N,vinte_e_cinco),
               produto(N,desodorante),

%A mulher mais velha está exatamente à esquerda da capricorniana
               exat_a_esquerda(O,P,S),
               signo(P,capricorniana),
               idade(O,vinte_e_oito),

%A garota de Capricórnio está ao lado da de Peixes.
               ao_lado(Q,R,S),
               signo(Q,capricorniana),
               signo(R,pisciana),

%A mulher de Peixes está ao lado da que foi comprar Desodorante.
               ao_lado(E58,T,S),
               signo(E58,pisciana),
               produto(T,desodorante),

%A mulher que nasceu em outubro está na quinta posição.
               quinta_da_fila(U,S),
               signo(U,escorpiao),

%A mulher de Azul está em algum lugar à esquerda da que foi comprar Hidratante.
               a_esquerda(V,X85,S),
               camiseta(V,azul),
               produto(X85,hidratante),

%A garota de Amarelo está em algum lugar à esquerda da que foi comprar Remédio.
               a_esquerda(Z,W,S),
               camiseta(Z,amarela),
               produto(W,remedio),

%Na terceira posição está quem precisa de Remédio.
               terceira_da_fila(Y85,S),
               produto(Y85,remedio),

%A mulher que foi comprar Desodorante está em algum lugar à direita da que está de Amarelo.
               a_direita(A1,B1,S),
               produto(A1,desodorante),
               camiseta(B1,amarela),

%A de Amarelo está em algum lugar entre a que precisa de Xampu e a Gabriela, nessa ordem.
               entre(C1,D1,E1,S),
               camiseta(C1,amarela),
               produto(D1,xampu),
               nome(E1,gabriela),
...
%A capricorniana e a Gabriela estão lado a lado.
               lado_a_lado(G1,H1,S),
               signo(G1,capricorniana),
               nome(H1,gabriela),

%A mulher do signo de Capricórnio está exatamente à esquerda da que usa Rabo de Cavalo.
               exat_a_esquerda(I1,J1,S),
               signo(I1,capricorniana),
               cabelo(J1,rabo_de_cavalo),

%Quem tem o cabelo Desfiado está exatamente à direita da que está de Azul.
               exat_a_direita(K1,L1,S),
               cabelo(K1,desfiado),
               camiseta(L1,azul),

%Na quinta posição está a mulher de Chanel.
               quinta_da_fila(M1,S),
               cabelo(M1,chanel),

%Restriçoes de Fechamento
               pessoa(N1,S),
               nome(N1,fernanda),
